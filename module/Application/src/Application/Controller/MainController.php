...
class MainController  extends AbstractActionController
	...
	public function fotoSaveUrl($url){
		...
		if (mb_stristr($typeFoto,'jpeg', 0, 'UTF-8') or
			mb_stristr($typeFoto,'jpg', 0, 'UTF-8') or
			mb_stristr($typeFoto,'png', 0, 'UTF-8') or
			mb_stristr($typeFoto,'gif', 0, 'UTF-8')
		) {
	...
	{
		$name = "";
		if (isset($file['type']) and ($file['type'] == "image/jpeg" or
				$file['type'] == "image/png" or
				$file['type'] == "image/gif"    or $file['type'] == "image/jpg")
		) {
			$uploaddir = $_SERVER['DOCUMENT_ROOT'] . '/foto/original/';
	...
	public function fotoSize($foto,$width,$dirname,$return=false){
		...
		switch($size[2]){
			case '1': $source=imagecreatefromgif($foto);
				break;
			case '2': $source=imagecreatefromjpeg($foto);
				break;
			case '3': $source=imagecreatefrompng($foto);
				break;
			default: return false;
		}
		$substrate=imagecreatetruecolor($width,$hieght);
		imagecopyresized($substrate,$source,0,0,0,0,$width,$hieght,$size[0],$size[1]);
	...
	private function setMimeType($link){
		...
		$mask['docx'] = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
	...
	public function email4($connect = 'local', $title = "",$to = "", $from = "", $html = "", $file = ""){
		...
		if(!empty($file)){
			foreach($file as $k=>$v){
				try{
					...
					$attachment = new MimePart(fopen($_SERVER['DOCUMENT_ROOT']."/".$v['link'], 'r'));
					if(!$this->setMimeType($v['link']))return false;
