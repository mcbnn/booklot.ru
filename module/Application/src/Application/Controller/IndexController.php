...
class IndexController extends AbstractActionController
{
    ...
    public function starsAction(){
        ...
        try{
            ...
            $check =  $sm->get('Application\Model\StarsTable')->fetchAll(false, false, ['id_book' => $id_book, 'ip' => $ip]);
            ...
            if($check->count() == 0){
                ...
                $sm->get('Application\Model\StarsTable')->save($arr, ['id_book' => $id_book, 'ip' => $ip]);
            }
        ...
        return new JsonModel(['stars' => $aver_value, 'count' => $count, 'err' => $err]);
    ...
    public function searchAction(){
        ...
        if($where){
            ...
            $book = $sm->get('Application\Model\BookTable')
                ...
                ->joinTranslitLeft()
                ->joinMTranslitLeft()
                ->joinColumn(array('id', 'foto', 'alias', 'visit', 'name', 'text_small', 'stars', 'count_stars', 'date_add'))
                ->fetchAll(false, 'book.name ASC', $where, 100, 'book.id');
            $count = $book->count();
        }

       return new ViewModel(array('book' => $book, 'count' => $count));
    }
    ...
    public function citCommAction()
    {
        ...
        if (!$user) return new JsonModel(['error' => 1, 'text' => 'Проблемы с добавлением, возможно вам нужно авторизоваться']);
        ...
        $arr['ban'] = 0;
        $arr['vis'] = 1;
        if (mb_strlen($arr['text'], "UTF-8") < 3) return new JsonModel(['error' => 1, 'text' => 'Проблемы с добавлением, комментарий должен быть больше']);
        $id = $sm->get('Application\Model\CommentsTable')->save($arr, false, true);
        $text = $this->templateComm($arr['id_content']);
        return new JsonModel(['error' => 0, 'text' => $text, 'count_comm' => $this->count_comm]);
    }
    ...
    public function delAction()
    {
        ...
        $user = $this->getServiceLocator()->get('AuthService')->getIdentity();
        if (!$user) return new JsonModel(['error' => 1, 'text' => 'Проблемы с удаление, возможно вам нужно авторизоваться']);
        $sm = $this->getServiceLocator();
        ...
        if ($check->count() == 0 or $check->count() > 1) return new JsonModel(['error' => 1, 'text' => 'Проблемы с удаление, возможно вам нужно авторизоваться']);
    ...
    {
        $id = $this->params()->fromPost('id');
        if (!is_numeric($id)) return new JsonModel(['error' => 1, 'text' => 'Проблема с отображением комментариев']);
    ...
    public function editAction()
    {
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                return $this->redirect()->toRoute('home/slash/edit', array('subdomain' => $site));
    ...
    public function riderAction()
    {
        ...
        $sm = $this->getServiceLocator();
        if ($rider == 'zhanr') {
            return $this->redirect()->toRoute('home/genre', array('subdomain' => $site))->setStatusCode(301);
            ...
            if ($id) {
                ...
                if ($avtor) {
                    ...
                    if ($author->count() != 0) {
                        ...
                        return $this->redirect()->toRoute('home/authors/one', array('subdomain' => $site, 'alias_menu' => $author->alias))->setStatusCode(301);
                    }
                } elseif ($zhanr) {
                    $order = "id_main ASC";
                    ...
                    return $this->redirect()->toRoute('home/genre/one', array('subdomain' => $site, 's' => $bookRoute['s'], 'alias_menu' => $bookRoute['alias_menu']))->setStatusCode(301);
                } elseif ($serii) {
                    $where = "m_serii.id = '$id'";
                    ...
                    if ($serii->count() != 0) {
                        $serii = $serii->current();
                        return $this->redirect()->toRoute('home/series/one', array('subdomain' => $site, 'alias_menu' => $serii->alias))->setStatusCode(301);
                    }
                } elseif ($translit) {
                    ...
                    if ($translit->count() != 0) {
                        ...
                        return $this->redirect()->toRoute('home/translit/one', array('subdomain' => $site, 'alias_menu' => $translit->alias))->setStatusCode(301);
                    }
                }
            }

            return $this->redirect()->toRoute('home', array('subdomain' => $site))->setStatusCode(301);
            ...
            if ($id) {
                ...
                if ($book->count() != 0) {
                    $book = $book->current();
                    return $this->redirect()->toRoute('home/genre/one/book', array('subdomain' => $site, 's' => $book->n_s, 'alias_menu' => $book->n_alias_menu, 'book' => $book->alias))->setStatusCode(301);
    ...
    public function authorsAction()
    {
        ...
        $authors = $sm->get('Application\Model\MAvtorTable')
            ->fetchAll(true, $order, $where, false);
        $authors->setCurrentPageNumber((int)$this->params()->fromRoute('page', 1));
        $authors->setItemCountPerPage(200);

        $where = "route = 'home/authors'";
        $menu = $sm->get('Application\Model\MZhanrTable')
            ->fetchAll(false, false, $where)->current();
        $this->seo("Авторы", "Авторы", $menu->description, $menu->keywords);

        return new ViewModel(array('authors' => $authors, 'menu' => $menu));
    }
    ...
    public function translitAction()
    {
        ...
        return new ViewModel(array('translit' => $translit, 'menu' => $menu));
    }
    ...
    public function seriesoneAction()
    {
        ...
        return new ViewModel(array('book' => $book, 'title' => $t));
    }
    ...
    public function seriesAction()
    {
        ...
        $series = $sm->get('Application\Model\MSeriiTable')
            ->fetchAll(true, $order, $where, false);
        ...
        return new ViewModel(array('series' => $series, 'menu' => $menu));
    }
    ...
    public function authorAction()
    {
        ...
        $avtor = $sm->get('Application\Model\MAvtorTable')
            ->fetchAll(false, false, $where)->current();
    ...
    public function translitoneAction()
    {
        ...
        $where = "alias = '$alias_menu'";
        $avtor = $sm->get('Application\Model\MTranslitTable')
            ->fetchAll(false, false, $where)->current();
    ...
    public function indexAction()
    {
        ...
        if($sort and in_array($sort,['visit','name', 'date_add', 'stars'])){
        ...
        $vm = new ViewModel(array('book' => $book, 'pag' => $pag));
    ...
    public function bookAction()
    {
        ...
        $files = $sm->get('Application\Model\BookFilesTable')
            ->fetchAll(false, false, $where);
        ...
        $id_menu = $sm->get('Application\Model\ZhanrTable')
            ->fetchAll(false, false, $where);
	...
	//текст
    	$where = "id_main = '{$book['id']}'";
        $count_text = $sm->get('Application\Model\TextTable')
            ->fetchAll(false, false, $where, 1);
	...
	$count_text = $count_text->count();
        ...
        $similar = $sm->get('Application\Model\MSeriiTable')
            ->joinSerii()
            ->joinBook()
            ->fetchAll(false, false, $where, 3);
        //	var_dump($similar->current());die();
	...
	$count_text = $count_text->count();
        ...
        $similar = $sm->get('Application\Model\MAvtorTable')
            ->joinAvtor()
            ->joinBook()
            ->fetchAll(false, false, $where, 3);
        //	var_dump($similar->current());die();
    ...
    public function readAction()
    {
        ...
        if (!$page_str) {
            ...
            $vm = new ViewModel(array('title' => $t));
        ...
        $text = $sm->get('Application\Model\TextTable')
            ->fetchAll(true, false, $where);
    ...
    public function oneGenreAction()
    {
        ...
        return new ViewModel(array('book' => $book, 'menu' => $menu, 'pag' => $pag));
    }

    public function notSearch($search)
    {

        $vm = new ViewModel(array('search' => $search));
